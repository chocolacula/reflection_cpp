...
struct TypeHelper<std::vector<T>> {
  ...
  static TypeInfo reflect(void* value, bool is_const) {
    return TypeInfo(Sequence(static_cast<std::vector<T>*>(value), is_const));
  }

  static std::string_view type_name() {
    static auto name = format("std::vector<{}>", TypeHelper<T>::type_name());
    return name;
  }
  ...
  static void copy(void* to, const void* from) {
    *static_cast<std::vector<T>*>(to) = *static_cast<const std::vector<T>*>(from);
  }
