...
struct StdSet : public ISet {
  ...
  Expected<None> push(Var value) override {
    ...
    if (nested_type != value.type()) {
      return Error(fmt::format("Trying to set with type: {} to set<{}>",  //
                               TypeActions::type_name(value.type()),      //
                               TypeActions::type_name(nested_type)));
    }
  ...
  Expected<None> remove(Var value) override {
    ...
    if (nested_type != value.type()) {
      return Error(fmt::format("Cannot remove value with type: {} from set<{}>",  //
                               TypeActions::type_name(value.type()),              //
                               TypeActions::type_name(nested_type)));
    }
    _set->pop();
