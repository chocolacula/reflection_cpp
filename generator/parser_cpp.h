...
class ParserCpp {
 public:
  ParserCpp(const CompilationDatabase* compdb, std::string&& file_path)
      : _tool(*compdb, {file_path}),  //
        _factory(&_ctx) {

    _ctx.origin_file = &(_tool.getSourcePaths()[0]);
  }
  ...
  class Matcher : public MatchFinder::MatchCallback {
   ...
   private:
    ...
    void handle_class(const CXXRecordDecl* c, AttrReflect* attr, const SourceManager& sm, const LangOptions& opts) {
      ...
      _ctx->result.emplace_back(std::move(json));
    }
    ...
    static std::string to_access_string(AccessSpecifier access) {
      // fast convert to rr/info/access.h Access enum constants string representation
      switch (access) {
